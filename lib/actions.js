...
Actions.prototype.setup = function() {
  ...
  for(var os in self.sessionsMap) {
    ...
    var taskList = sessionsInfo.taskListsBuilder.setup(self.config);
...
Actions.prototype.deploy = function() {
  ...
  buildApp(buildLocation, function(err) {
    if(err) {
      process.exit(1);
    } else {
      var sessionsData = [];
      ...
      async.mapSeries(
        ...
        whenAfterCompleted(buildLocation)
      )
    }
  });
};
...
Actions.prototype.restart = function() {
  ...
  for(var os in self.sessionsMap) {
    var sessionsInfo = self.sessionsMap[os];
    var taskList = sessionsInfo.taskListsBuilder.restart(self.config.appName);
    taskList.run(sessionsInfo.sessions);
  }
};
...
Actions.prototype.stop = function() {
  ...
  for(var os in self.sessionsMap) {
    var sessionsInfo = self.sessionsMap[os];
    var taskList = sessionsInfo.taskListsBuilder.stop(self.config.appName);
    taskList.run(sessionsInfo.sessions);
  }
};
...
Actions.prototype.start = function() {
  ...
  for(var os in self.sessionsMap) {
    var sessionsInfo = self.sessionsMap[os];
    var taskList = sessionsInfo.taskListsBuilder.start(self.config.appName);
    taskList.run(sessionsInfo.sessions);
  }
};
...
function whenAfterCompleted(buildLocation) {
